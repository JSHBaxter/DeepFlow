...
int main(int argc, char** argv){
    ...
    for(int b = 0; b < n_b; b++)
	    set_buffer(dev, data_buf+b*n_s+(n_s-1), 1.0, 1);
	...
	for(int b = 0; b < n_b; b++){
		auto solver_auglag = POTTS_AUGLAG_GPU_SOLVER_3D(dev, b, n_c, data_sizes, data_buf+b*n_s, rx_buf+b*n_s, ry_buf+b*n_s, rz_buf+b*n_s, u_buf+b*n_s, buffers_full, buffers_img);
		solver_auglag();
        ...
        std::cout << "\nrx_buf" << std::endl;
        print_gpu_buffer(dev,rx_buf+b*n_s,n_s);
        std::cout << "\nry_buf" << std::endl;
        print_gpu_buffer(dev,ry_buf+b*n_s,n_s);
        std::cout << "\nrz_buf" << std::endl;
		...
		auto solver_meanpass = POTTS_MEANPASS_GPU_SOLVER_3D(dev, b, n_c, data_sizes, data_buf+b*n_s, rx_buf+b*n_s, ry_buf+b*n_s, rz_buf+b*n_s, u_buf+b*n_s, out_buf+b*n_s, buffers_full);
		solver_meanpass();
	...
	//get the temporary buffers
	num_buffers_full = POTTS_MEANPASS_GPU_GRADIENT_3D::num_buffers_full();
	buffer = 0;
	...
	for(int b = 0; b < n_b; b++){
		auto grad_meanpass = POTTS_MEANPASS_GPU_GRADIENT_3D(dev, b, n_c, data_sizes, out_buf+b*n_s, g_buf+b*n_s, rx_buf+b*n_s, ry_buf+b*n_s, rz_buf+b*n_s, g_data_buf+b*n_s, g_rx_buf+b*n_s, g_ry_buf+b*n_s, g_rz_buf+b*n_s, buffers_full);
		grad_meanpass();

...
#include "binary_auglag1d_cpu_solver.cc"

// If we are using CUDA, include the GPU kernels
#if GOOGLE_CUDA
#include "binary_auglag1d_gpu_solver.cc"
#endif

// Define the OpKernel class
// template parameter <float> is the datatype of the tensors.
template <typename Device>
class BinaryAuglag1dOp : public OpKernel {
public:
    explicit BinaryAuglag1dOp(OpKernelConstruction* context) : OpKernel(context) {}
    ...
    void Compute(OpKernelContext* context) override {
        ...
        int num_intermediates_full = BinaryAuglag1dFunctor<Device>().num_buffers_full();
        int num_intermediates_images = BinaryAuglag1dFunctor<Device>().num_buffers_images();

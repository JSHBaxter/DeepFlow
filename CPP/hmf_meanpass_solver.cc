...
HMF_MEANPASS_SOLVER<DEV>::HMF_MEANPASS_SOLVER(
    const DEV & dev,
...
num_children(0)
{
    if(DEBUG_ITER) std::cout << "HMF_MEANPASS_SOLVER Constructor " << dim << " " << n_s << " " << n_c << " " << n_r << " " << std::endl;    
...
template<typename DEV>
void HMF_MEANPASS_SOLVER<DEV>::allocate_buffers(float* buffer, float** const carry_over, const float** const c_carry_over){
    if(DEBUG_ITER) std::cout << "HMF_MEANPASS_SOLVER Allocate " << std::endl;
    r_eff = buffer;
    ...
    u_ind = (float**) allocate_memory(MAXFLOW_ALGORITHM<DEV>::dev, 2*n_r*(sizeof(float*)+sizeof(float))); //get some custom memory (more robust to type size changes)
    ...
    move_memory_to_device(MAXFLOW_ALGORITHM<DEV>::dev, tmp_u_ind, u_ind, n_r*sizeof(float*));
    delete tmp_u_ind;
    ...
    move_memory_to_device(MAXFLOW_ALGORITHM<DEV>::dev, tmp_r_ind, reg_ind, n_r*sizeof(float*));
    delete tmp_r_ind;
    
    //get number of children (save as float)
    num_children = (float*)((void*)(reg_ind + n_r));
...
template<typename DEV>
int HMF_MEANPASS_SOLVER<DEV>::get_buffer_size(){
...
HMF_MEANPASS_SOLVER<DEV>::~HMF_MEANPASS_SOLVER(){
    delete spatial_flow;
    deallocate_memory(MAXFLOW_ALGORITHM<DEV>::dev, (void**)&u_ind);
}

template<typename DEV>
void HMF_MEANPASS_SOLVER<DEV>::block_iter(const int parity){
    ...
    for (int l = n_c; l < n_r; l++) {
        ...
        for(int c = 0; c < n->c; c++)
            inc_buffer(MAXFLOW_ALGORITHM<DEV>::dev, u_full+n->children[c]->r*n_s, u_full+n->r*n_s, n_s);
    }
    if(DEBUG_PRINT){ std::cout << "u_full: "; print_buffer(MAXFLOW_ALGORITHM<DEV>::dev, u_full, n_s*n_r);}
    //aggregate_bottom_up(MAXFLOW_ALGORITHM<DEV>::dev, u_ind, u_full, u, n_s, n_c, n_r);

    //calculate the effective regularization (overwrites own r_eff)
    spatial_flow->run();
    if(DEBUG_PRINT){ std::cout << "r_eff: "; print_buffer(MAXFLOW_ALGORITHM<DEV>::dev, r_eff, n_s*n_r);}
    //print_buffer(MAXFLOW_ALGORITHM<DEV>::dev, r_eff, n_s*n_r);

    //calculate the aggregate effective regularization (overwrites own r_eff)
    for (int l = n_r-1; l >= 0; l--) {
        const TreeNode* n = bottom_up_list[l];
        for(int c = 0; c < n->c; c++)
            inc_buffer(MAXFLOW_ALGORITHM<DEV>::dev, r_eff+n->r*n_s, r_eff+n->children[c]->r*n_s, n_s);
    }
    if(DEBUG_PRINT){ std::cout << "r_eff: "; print_buffer(MAXFLOW_ALGORITHM<DEV>::dev, r_eff, n_s*n_r);}
    //print_buffer(MAXFLOW_ALGORITHM<DEV>::dev, r_eff, n_s*n_r);
...
template<typename DEV>
void HMF_MEANPASS_SOLVER<DEV>::run(){
    ...
    for(int i = 0; i < max_loop; i++){    
        ...
        if(DEBUG_ITER) std::cout << "HMF_MEANPASS_SOLVER Iter #" << i << ": " << max_change << std::endl;
...
template<typename DEV>
HMF_MEANPASS_GRADIENT<DEV>::HMF_MEANPASS_GRADIENT(
    const DEV & dev,
    TreeNode** bottom_up_list,
    const int dim,
    const int* dims,
...
tmp(0)
{
    if(DEBUG_ITER) std::cout << "HMF_MEANPASS_GRADIENT Constructor " << dim << " " << n_s << " " << n_c << " " << n_r << std::endl;
    spatial_flow = new SPATIAL_MEANPASS_BACKWARD_SOLVER<DEV>(MAXFLOW_ALGORITHM<DEV>::dev, n_r, dims, dim, inputs+2, outputs+1);
    MAXFLOW_ALGORITHM<DEV>::construct();
}

template<typename DEV>
void HMF_MEANPASS_GRADIENT<DEV>::allocate_buffers(float* buffer, float** const carry_over, const float** const c_carry_over){
    if(DEBUG_ITER) std::cout << "HMF_MEANPASS_GRADIENT Allocate " << std::endl;
    u = buffer;
    dy = u + n_s*n_r;
    du = dy + n_s*n_r;
    tmp = du + n_s*n_r;
...
template<typename DEV>
int HMF_MEANPASS_GRADIENT<DEV>::get_buffer_size(){
...
template<typename DEV>
void HMF_MEANPASS_GRADIENT<DEV>::block_iter(){
    ...
    for (int l = 0; l < n_r; l++) {
        const TreeNode* n = bottom_up_list[l];
        for(int c = 0; c < n->c; c++)
            inc_buffer(MAXFLOW_ALGORITHM<DEV>::dev, dy+n->children[c]->r*n_s, dy+n->r*n_s, n_s);
    }
    ...
    for (int l = n_r-1; l >= 0; l--) {
        const TreeNode* n = bottom_up_list[l];
        for(int c = 0; c < n->c; c++)
            inc_buffer(MAXFLOW_ALGORITHM<DEV>::dev, du+n->r*n_s, du+n->children[c]->r*n_s, n_s);
    }
    if(DEBUG_PRINT){ std::cout << "du: "; print_buffer(MAXFLOW_ALGORITHM<DEV>::dev, du, n_s*n_r);}
}

template<typename DEV>
void HMF_MEANPASS_GRADIENT<DEV>::run(){
    if(DEBUG_ITER) std::cout << "HMF_MEANPASS_GRADIENT Init" << std::endl;
    ...
    for (int l = n_c; l < n_r; l++) {
        ...
        if(DEBUG_PRINT){ std::cout << "\t node: " << n->r << "\tnum_children:\t" << n->c << std::endl; }
        for(int c = 0; c < n->c; c++)
            inc_buffer(MAXFLOW_ALGORITHM<DEV>::dev, u+n->children[c]->r*n_s, u+n->r*n_s, n_s);
    }
    if(DEBUG_PRINT){ std::cout << "u: "; print_buffer(MAXFLOW_ALGORITHM<DEV>::dev, u, n_s*n_r);}
    ...
    for(int i = 0; i < max_loop; i++){
        ...
        if(DEBUG_ITER) std::cout << "HMF_MEANPASS_GRADIENT Iter " << i << " " << max_change << std::endl;

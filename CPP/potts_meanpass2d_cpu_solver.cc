...
public:
    ...
    void operator()(){
        ...
        clear(g_rx,g_ry,n_s*n_c);
        get_reg_gradients(grad, u, g_rx, g_ry, n_x, n_y, n_c, 1.0f);
         
        //psuh gradient back an iteration
        get_gradient_for_u(grad, rx, ry, g_u, n_x, n_y, n_c, 1);
        ...
        for(int i = 0; i < max_loops; i++){
            for(int iter = 0; iter < min_iters; iter++){
                ...
                get_reg_gradients(dy, u, g_rx, g_ry, n_x, n_y, n_c, tau);

                //push back gradient 
                get_gradient_for_u(dy, rx, ry, g_u, n_x, n_y, n_c, tau);
            }
...
struct PottsMeanpass2dFunctor<CPUDevice> {
  void operator()(
    ...
    for(int b = 0; b < n_batches; b++)
        threads[b] = new std::thread(PottsMeanpass2d_SolverBatchThreadChannelsLast(b, sizes, data_cost+ b*n_s*n_c,
																																									  rx_cost+ b*n_s*n_c,
																																									  ry_cost+ b*n_s*n_c,
																																									  u+ b*n_s*n_c));
    for(int b = 0; b < n_batches; b++)
...
struct PottsMeanpass2dGradFunctor<CPUDevice> {
  void operator()(
    ...
    std::thread** threads = new std::thread* [n_batches];
    for(int b = 0; b < n_batches; b++)
        threads[b] = new std::thread(PottsMeanpass2d_GradientBatchThreadChannelsLast(b, sizes, u+ b*n_s*n_c,
																																										  g+ b*n_s*n_c,
																																										  rx_cost+ b*n_s*n_c,
																																										  ry_cost+ b*n_s*n_c,
																																										  g_data+ b*n_s*n_c,

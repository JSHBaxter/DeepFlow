...
void HMF_MEANPASS_CPU_SOLVER_2D::clean_up(){
}

void HMF_MEANPASS_CPU_SOLVER_2D::update_spatial_flow_calc(){
    calculate_r_eff_channels_first(r_eff, rx_b, ry_b, u_tmp, n_x, n_y, n_r);
}

void HMF_MEANPASS_CPU_SOLVER_2D::parity_mask_buffer(float* buffer, const int parity){
    parity_mask_channels_first(buffer,n_x,n_y,n_c,parity);
}

void HMF_MEANPASS_CPU_SOLVER_2D::parity_merge_buffer(float* buffer, const float* other, const int parity){
    parity_merge_channels_first(buffer,other,n_x,n_y,n_c,parity);
}

HMF_MEANPASS_CPU_SOLVER_2D::~HMF_MEANPASS_CPU_SOLVER_2D(){
...
void HMF_MEANPASS_CPU_GRADIENT_2D::clean_up(){
    if(!channels_first){
        float* temp = new float[n_r*n_s];

...
public:
	...
	BINARY_MEANPASS_CPU_GRADIENT_BASE(batch, sizes[1]*sizes[2]*sizes[3], sizes[4], u, g, g_d),
	n_x(sizes[1]),
	n_y(sizes[2]),
	n_z(sizes[3]),
	rx(rx_cost),
...
struct BinaryMeanpass3dFunctor<CPUDevice> {
  void operator()(
    ...
    for(int b = 0; b < n_batches; b++)
        threads[b] = new std::thread(BINARY_MEANPASS_CPU_SOLVER_3D(b, sizes,
																  data_cost + b*n_s*n_c,
																  rx_cost + b*n_s*n_c,
																  ry_cost + b*n_s*n_c,
																  rz_cost + b*n_s*n_c,
...
struct BinaryMeanpass3dGradFunctor<CPUDevice> {
  void operator()(
    ...
    std::thread** threads = new std::thread* [n_batches];
    for(int b = 0; b < n_batches; b++)
        threads[b] = new std::thread(BINARY_MEANPASS_CPU_GRADIENT_3D(b, sizes,
																	u + b*n_s*n_c,
																	g + b*n_s*n_c,
																	rx_cost + b*n_s*n_c,
																	ry_cost + b*n_s*n_c,

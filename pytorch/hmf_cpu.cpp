...
void hmf_auglag_1d_cpu(torch::Tensor data, torch::Tensor rx, torch::Tensor out, torch::Tensor parentage) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver = new HMF_AUGLAG_SOLVER<CPU_DEVICE>(dev, bottom_up_list, false, 1, data_sizes, n_c, n_r, inputs, out_buf+b*n_s);
		solver->run();
        delete solver;
	}
...
void hmf_auglag_2d_cpu(torch::Tensor data, torch::Tensor rx, torch::Tensor ry, torch::Tensor out, torch::Tensor parentage) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver = new HMF_AUGLAG_SOLVER<CPU_DEVICE>(dev, bottom_up_list, false, 2, data_sizes, n_c, n_r, inputs, out_buf+b*n_s);
		solver->run();
        delete solver;
	}
	//free temporary memory
...
void hmf_auglag_3d_cpu(torch::Tensor data, torch::Tensor rx, torch::Tensor ry, torch::Tensor rz, torch::Tensor out, torch::Tensor parentage) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver = new HMF_AUGLAG_SOLVER<CPU_DEVICE>(dev, bottom_up_list, false, 3, data_sizes, n_c, n_r, inputs, out_buf+b*n_s);
		solver->run();
        delete solver;
	}
...
void hmf_meanpass_1d_cpu(torch::Tensor data, torch::Tensor rx, torch::Tensor out, torch::Tensor parentage) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver_auglag = new HMF_AUGLAG_SOLVER<CPU_DEVICE>(dev, bottom_up_list, false, 1, data_sizes, n_c, n_r, buffers+1, u_init_buf);
		solver_auglag->run();
        delete solver_auglag;
		exp(dev,u_init_buf,u_init_buf,n_s);
		
		auto solver_meanpass = new HMF_MEANPASS_SOLVER<CPU_DEVICE>(dev, bottom_up_list, 1, data_sizes, n_c, n_r, buffers, out_buf+b*n_s);
		solver_meanpass->run();
...
void hmf_meanpass_1d_cpu_back(torch::Tensor g, torch::Tensor u, torch::Tensor rx, torch::Tensor g_data, torch::Tensor g_rx, torch::Tensor parentage) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto grad_meanpass = new HMF_MEANPASS_GRADIENT<CPU_DEVICE>(dev, bottom_up_list, 1, data_sizes, n_c, n_r, inputs, outputs);
		grad_meanpass->run();
...
void hmf_meanpass_2d_cpu(torch::Tensor data, torch::Tensor rx, torch::Tensor ry, torch::Tensor out, torch::Tensor parentage) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver_auglag = new HMF_AUGLAG_SOLVER<CPU_DEVICE>(dev, bottom_up_list, false, 2, data_sizes, n_c, n_r, buffers+1, u_init_buf);
		solver_auglag->run();
        delete solver_auglag;
		exp(dev,u_init_buf,u_init_buf,n_s);
		
		auto solver_meanpass = new HMF_MEANPASS_SOLVER<CPU_DEVICE>(dev, bottom_up_list, 2, data_sizes, n_c, n_r, buffers, out_buf+b*n_s);
		solver_meanpass->run();
		delete solver_meanpass;
	}
...
void hmf_meanpass_2d_cpu_back(torch::Tensor g, torch::Tensor u, torch::Tensor rx, torch::Tensor ry, torch::Tensor g_data, torch::Tensor g_rx, torch::Tensor g_ry, torch::Tensor parentage) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto grad_meanpass = new HMF_MEANPASS_GRADIENT<CPU_DEVICE>(dev, bottom_up_list, 2, data_sizes, n_c, n_r, inputs, outputs);
		grad_meanpass->run();
        delete grad_meanpass;
	}
...
void hmf_meanpass_3d_cpu(torch::Tensor data, torch::Tensor rx, torch::Tensor ry, torch::Tensor rz, torch::Tensor out, torch::Tensor parentage) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver_auglag = new HMF_AUGLAG_SOLVER<CPU_DEVICE>(dev, bottom_up_list, false, 3, data_sizes, n_c, n_r, buffers+1, u_init_buf);
		solver_auglag->run();
        delete solver_auglag;
		exp(dev,u_init_buf,u_init_buf,n_s);
		
		auto solver_meanpass = new HMF_MEANPASS_SOLVER<CPU_DEVICE>(dev, bottom_up_list, 3, data_sizes, n_c, n_r, buffers, out_buf+b*n_s);
		solver_meanpass->run();
        delete solver_meanpass;
	}
...
void hmf_meanpass_3d_cpu_back(torch::Tensor g, torch::Tensor u, torch::Tensor rx, torch::Tensor ry, torch::Tensor rz, torch::Tensor g_data, torch::Tensor g_rx, torch::Tensor g_ry, torch::Tensor g_rz, torch::Tensor parentage) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto grad_meanpass = new HMF_MEANPASS_GRADIENT<CPU_DEVICE>(dev, bottom_up_list, 3, data_sizes, n_c, n_r, inputs, outputs);
		grad_meanpass->run();
        delete grad_meanpass;
	}
...
void hmf_cpu_bindings(py::module & m) {
  m.def("hmf_auglag_1d_cpu_forward", &hmf_auglag_1d_cpu, "deepflow hmf_auglag_1d_cpu_forward");
  m.def("hmf_auglag_2d_cpu_forward", &hmf_auglag_2d_cpu, "deepflow hmf_auglag_2d_cpu_forward");
  m.def("hmf_auglag_3d_cpu_forward", &hmf_auglag_3d_cpu, "deepflow hmf_auglag_3d_cpu_forward");
  m.def("hmf_meanpass_1d_cpu_forward", &hmf_meanpass_1d_cpu, "deepflow hmf_meanpass_1d_cpu_forward");
  m.def("hmf_meanpass_2d_cpu_forward", &hmf_meanpass_2d_cpu, "deepflow hmf_meanpass_2d_cpu_forward");
  m.def("hmf_meanpass_3d_cpu_forward", &hmf_meanpass_3d_cpu, "deepflow hmf_meanpass_3d_cpu_forward");
  m.def("hmf_meanpass_1d_cpu_backward", &hmf_meanpass_1d_cpu_back, "deepflow hmf_meanpass_1d_cpu_backward");
  m.def("hmf_meanpass_2d_cpu_backward", &hmf_meanpass_2d_cpu_back, "deepflow hmf_meanpass_2d_cpu_backward");
  m.def("hmf_meanpass_3d_cpu_backward", &hmf_meanpass_3d_cpu_back, "deepflow hmf_meanpass_3d_cpu_backward");
}
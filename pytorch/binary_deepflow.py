...
class Binary_MAP1d(torch.autograd.Function):
    ...
    def forward(ctx, d, rx):
        ...
        if len(d.shape) == 3 and len(rx.shape) == 3:
            ...
            if d.is_cuda:
                binary_gpu.auglag_1d_forward(d,rx, output)
            else:
                binary_cpu.auglag_1d_forward(d,rx, output)
            return output
        else:
...
class Binary_MAP2d(torch.autograd.Function):
    ...
    def forward(ctx, d, rx, ry):
        if len(d.shape) == 4 and len(rx.shape) == 4 and len(ry.shape) == 4:
            output = torch.zeros_like(d)
            if d.is_cuda:
                binary_gpu.auglag_2d_forward(d,rx, ry, output)
            else:
                binary_cpu.auglag_2d_forward(d,rx, ry, output)
            return output
        else:
...
class Binary_MAP3d(torch.autograd.Function):
    ...
    def forward(ctx, d, rx, ry, rz):
        if len(d.shape) == 5 and len(rx.shape) == 5 and len(ry.shape) == 5 and len(rz.shape) == 5:
            output = torch.zeros_like(d)
            if d.is_cuda:
                binary_gpu.auglag_3d_forward(d, rx, ry, rz, output)
            else:
                binary_cpu.auglag_3d_forward(d, rx, ry, rz, output)
            return output
        else:
...
class Binary_Mean1d(torch.autograd.Function):
    ...
    def forward(ctx, d, rx):
        if len(d.shape) == 3 and len(rx.shape) == 3:
            output = torch.zeros_like(d)
            if d.is_cuda:
                binary_gpu.meanpass_1d_forward(d,rx, output)
            else:
                binary_cpu.meanpass_1d_forward(d,rx, output)
    ...
    def backward(ctx, grad_output):
        ...
        rx = rx.clone()
        if d.is_cuda:
            binary_gpu.meanpass_1d_backward(grad_output, u, rx, grad_d, grad_rx)
        else:
            binary_cpu.meanpass_1d_backward(grad_output, u, rx, grad_d, grad_rx)
...
class Binary_Mean2d(torch.autograd.Function):
    ...
    def forward(ctx, d, rx, ry):
        if len(d.shape) == 4 and len(rx.shape) == 4 and len(ry.shape) == 4:
            output = torch.zeros_like(d)
            if d.is_cuda:
                binary_gpu.meanpass_2d_forward(d,rx, ry, output)
            else:
                binary_cpu.meanpass_2d_forward(d,rx, ry, output)
    ...
    def backward(ctx, grad_output):
        ...
        ry = ry.clone()
        if d.is_cuda:
            binary_gpu.meanpass_2d_backward(grad_output, u, rx, ry, grad_d, grad_rx, grad_ry)
        else:
            binary_cpu.meanpass_2d_backward(grad_output, u, rx, ry, grad_d, grad_rx, grad_ry)
...
class Binary_Mean3d(torch.autograd.Function):
    ...
    def forward(ctx, d, rx, ry, rz):
        if len(d.shape) == 5 and len(rx.shape) == 5 and len(ry.shape) == 5 and len(rz.shape) == 5:
            output = torch.zeros_like(d)
            if d.is_cuda:
                binary_gpu.meanpass_3d_forward(d,rx, ry, rz, output)
            else:
                binary_cpu.meanpass_3d_forward(d,rx, ry, rz, output)
    ...
    def backward(ctx, grad_output):
        ...
        print(u)
        if d.is_cuda:
            binary_gpu.meanpass_3d_backward(grad_output, u, rx, ry, rz, grad_d, grad_rx, grad_ry, grad_rz)
        else:
            binary_cpu.meanpass_3d_backward(grad_output, u, rx, ry, rz, grad_d, grad_rx, grad_ry, grad_rz)

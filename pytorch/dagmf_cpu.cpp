...
void dagmf_auglag_1d_cpu(torch::Tensor data, torch::Tensor rx, torch::Tensor out, torch::Tensor parentage) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver = new DAGMF_AUGLAG_SOLVER<CPU_DEVICE>(dev, false, bottom_up_list, 1, data_sizes, n_c, n_r, inputs, out_buf+b*n_s);
		solver->run();
        delete solver;
	}
...
void dagmf_auglag_2d_cpu(torch::Tensor data, torch::Tensor rx, torch::Tensor ry, torch::Tensor out, torch::Tensor parentage) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver = new DAGMF_AUGLAG_SOLVER<CPU_DEVICE>(dev, false, bottom_up_list, 2, data_sizes, n_c, n_r, inputs, out_buf+b*n_s);
		solver->run();
        delete solver;
	}
	//free temporary memory
...
void dagmf_auglag_3d_cpu(torch::Tensor data, torch::Tensor rx, torch::Tensor ry, torch::Tensor rz, torch::Tensor out, torch::Tensor parentage) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver = new DAGMF_AUGLAG_SOLVER<CPU_DEVICE>(dev, false, bottom_up_list, 3, data_sizes, n_c, n_r, inputs, out_buf+b*n_s);
		solver->run();
        delete solver;
	}
...
void dagmf_meanpass_1d_cpu(torch::Tensor data, torch::Tensor rx, torch::Tensor out, torch::Tensor parentage) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver_auglag = new DAGMF_AUGLAG_SOLVER<CPU_DEVICE>(dev, false, bottom_up_list, 1, data_sizes, n_c, n_r, buffers+1, u_init_buf);
		solver_auglag->run();
...
void dagmf_meanpass_2d_cpu(torch::Tensor data, torch::Tensor rx, torch::Tensor ry, torch::Tensor out, torch::Tensor parentage) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver_auglag = new DAGMF_AUGLAG_SOLVER<CPU_DEVICE>(dev, false, bottom_up_list, 2, data_sizes, n_c, n_r, buffers+1, u_init_buf);
		solver_auglag->run();
        delete solver_auglag;
		exp(dev,u_init_buf,u_init_buf,n_s);
...
void dagmf_meanpass_3d_cpu(torch::Tensor data, torch::Tensor rx, torch::Tensor ry, torch::Tensor rz, torch::Tensor out, torch::Tensor parentage) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver_auglag = new DAGMF_AUGLAG_SOLVER<CPU_DEVICE>(dev, false, bottom_up_list, 3, data_sizes, n_c, n_r, buffers+1, u_init_buf);
		solver_auglag->run();
        delete solver_auglag;
		exp(dev,u_init_buf,u_init_buf,n_s);

...
void hmf_auglag_1d_gpu(torch::Tensor data, torch::Tensor rx, torch::Tensor out, torch::Tensor parentage) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver = new HMF_AUGLAG_SOLVER<CUDA_DEVICE>(dev, bottom_up_list, false, 1, data_sizes, n_c, n_r, inputs, out_buf+b*n_s);
		solver->run();
        delete solver;
	}
...
void hmf_auglag_2d_gpu(torch::Tensor data, torch::Tensor rx, torch::Tensor ry, torch::Tensor out, torch::Tensor parentage) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver = new HMF_AUGLAG_SOLVER<CUDA_DEVICE>(dev, bottom_up_list, false, 2, data_sizes, n_c, n_r, inputs, out_buf+b*n_s);
		solver->run();
        delete solver;
	}
	//free temporary memory
...
void hmf_auglag_3d_gpu(torch::Tensor data, torch::Tensor rx, torch::Tensor ry, torch::Tensor rz, torch::Tensor out, torch::Tensor parentage) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver = new HMF_AUGLAG_SOLVER<CUDA_DEVICE>(dev, bottom_up_list, false, 3, data_sizes, n_c, n_r, inputs, out_buf+b*n_s);
		solver->run();
        delete solver;
	}
...
void hmf_meanpass_1d_gpu(torch::Tensor data, torch::Tensor rx, torch::Tensor out, torch::Tensor parentage) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver_auglag = new HMF_AUGLAG_SOLVER<CUDA_DEVICE>(dev, bottom_up_list, false, 1, data_sizes, n_c, n_r, buffers+1, u_init_buf);
		solver_auglag->run();
...
void hmf_meanpass_2d_gpu(torch::Tensor data, torch::Tensor rx, torch::Tensor ry, torch::Tensor out, torch::Tensor parentage) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver_auglag = new HMF_AUGLAG_SOLVER<CUDA_DEVICE>(dev, bottom_up_list, false, 2, data_sizes, n_c, n_r, buffers+1, u_init_buf);
		solver_auglag->run();
        delete solver_auglag;
		exp(dev,u_init_buf,u_init_buf,n_s);
...
void hmf_meanpass_3d_gpu(torch::Tensor data, torch::Tensor rx, torch::Tensor ry, torch::Tensor rz, torch::Tensor out, torch::Tensor parentage) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver_auglag = new HMF_AUGLAG_SOLVER<CUDA_DEVICE>(dev, bottom_up_list, false, 3, data_sizes, n_c, n_r, buffers+1, u_init_buf);
		solver_auglag->run();
        delete solver_auglag;
		exp(dev,u_init_buf,u_init_buf,n_s);

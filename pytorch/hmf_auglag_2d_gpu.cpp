...
void hmf_auglag_2d_gpu(torch::Tensor data, torch::Tensor rx, torch::Tensor ry, torch::Tensor out, torch::Tensor parentage, torch::Tensor data_index) {
	...
	for(int b = 0; b < n_b; b++){
		auto solver = HMF_AUGLAG_GPU_SOLVER_2D(dev, bottom_up_list, b, data_sizes, data_buf+b*n_s, rx_buf+b*n_sr, ry_buf+b*n_sr, out_buf+b*n_s, buffers_full, buffers_img);
		solver();
	}
...
PYBIND11_MODULE(hmf_auglag_2d_gpu, m) {
  m.def("forward", &hmf_auglag_2d_gpu, "hmf_auglag_2d_gpu forward");
}
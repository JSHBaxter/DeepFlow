...
void hmf_meanpass_2d_cpu(torch::Tensor data, torch::Tensor rx, torch::Tensor ry, torch::Tensor out, torch::Tensor parentage, torch::Tensor data_index) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver_auglag = HMF_AUGLAG_CPU_SOLVER_2D(bottom_up_list, b, data_sizes, data_buf+b*n_s, rx_buf+b*n_s, ry_buf+b*n_s, u_init_buf);
		solver_auglag();
		exp(u_init_buf,u_init_buf,n_s);
		
		auto solver_meanpass = HMF_MEANPASS_CPU_SOLVER_2D(bottom_up_list, b, data_sizes, data_buf+b*n_s, rx_buf+b*n_sr, ry_buf+b*n_sr, u_init_buf, out_buf+b*n_s);
		solver_meanpass();
...
void hmf_meanpass_2d_cpu_back(torch::Tensor g, torch::Tensor u, torch::Tensor rx, torch::Tensor ry, torch::Tensor g_data, torch::Tensor g_rx, torch::Tensor g_ry, torch::Tensor parentage, torch::Tensor data_index) {
	...
	int data_sizes [6] = {n_b,n_x,n_y,n_c,n_x,n_r};
	for(int b = 0; b < n_b; b++){
		auto grad_meanpass = HMF_MEANPASS_CPU_GRADIENT_2D(bottom_up_list, b, data_sizes, u_buf+b*n_s, g_buf+b*n_s, rx_buf+b*n_sr, ry_buf+b*n_sr, g_data_buf+b*n_s, g_rx_buf+b*n_sr, g_ry_buf+b*n_sr);
		grad_meanpass();
	}
...
PYBIND11_MODULE(hmf_meanpass_2d_cpu, m) {
  m.def("forward", &hmf_meanpass_2d_cpu, "hmf_meanpass_2d_cpu forward");
  m.def("backward", &hmf_meanpass_2d_cpu_back, "hmf_meanpass_2d_cpu backward");
}
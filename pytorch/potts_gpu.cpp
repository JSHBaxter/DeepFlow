...
void potts_auglag_1d_gpu(torch::Tensor data, torch::Tensor rx, torch::Tensor out) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver = new POTTS_AUGLAG_SOLVER<CUDA_DEVICE>(dev, false, 1, data_sizes, n_c, inputs, out_buff_curr);
		solver->run();
        delete solver;
	}
}
...
void potts_auglag_2d_gpu(torch::Tensor data, torch::Tensor rx, torch::Tensor ry, torch::Tensor out) {
	...
	for(int b = 0; b < n_b; b++){
        ...
        float* const out_buff_curr = out_buf + b*n_s*n_c;
		auto solver = new POTTS_AUGLAG_SOLVER<CUDA_DEVICE>(dev, false, 2, data_sizes, n_c, inputs, out_buff_curr);
		solver->run();
        delete solver;
	}
}
...
void potts_auglag_3d_gpu(torch::Tensor data, torch::Tensor rx, torch::Tensor ry, torch::Tensor rz, torch::Tensor out) {
	...
	for(int b = 0; b < n_b; b++){
        ...
        float* const out_buff_curr = out_buf + b*n_s*n_c;
		auto solver = new POTTS_AUGLAG_SOLVER<CUDA_DEVICE>(dev, false, 3, data_sizes, n_c, inputs, out_buff_curr);
		solver->run();
        delete solver;
	}
}
...
void potts_meanpass_1d_gpu(torch::Tensor data, torch::Tensor rx, torch::Tensor out) {
	...
	float* u_init_buf = 0;
    cudaMalloc(&u_init_buf, n_c*n_s*sizeof(float));
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver_auglag = new POTTS_AUGLAG_SOLVER<CUDA_DEVICE>(dev, false, 1, data_sizes, n_c, buffers+1, buffers[0]);
		solver_auglag->run();
		exp(dev,u_init_buf,u_init_buf,n_s*n_c);
        delete solver_auglag;
		
		auto solver_meanpass = new POTTS_MEANPASS_SOLVER<CUDA_DEVICE>(dev, 1, data_sizes, n_c, buffers, out_buf+b*n_s*n_c);
		solver_meanpass->run();
...
void potts_meanpass_1d_gpu_back(torch::Tensor g, torch::Tensor u, torch::Tensor rx, torch::Tensor g_data, torch::Tensor g_rx) {
    CUDA_DEVICE dev = { .dev_number = g.get_device(), .stream = c10::cuda::getCurrentCUDAStream(g.get_device()) };
	...
	for(int b = 0; b < n_b; b++){
		...
		auto grad_meanpass = new POTTS_MEANPASS_GRADIENT<CUDA_DEVICE>(dev, 1, data_sizes, n_c, inputs, outputs);
		grad_meanpass->run();
...
void potts_meanpass_2d_gpu(torch::Tensor data, torch::Tensor rx, torch::Tensor ry, torch::Tensor out) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver_auglag = new POTTS_AUGLAG_SOLVER<CUDA_DEVICE>(dev, false, 2, data_sizes, n_c, buffers+1, buffers[0]);
		solver_auglag->run();
		exp(dev,u_init_buf,u_init_buf,n_s*n_c);
        delete solver_auglag;
		
		auto solver_meanpass = new POTTS_MEANPASS_SOLVER<CUDA_DEVICE>(dev, 2, data_sizes, n_c, buffers, out_buf+b*n_s*n_c);
		solver_meanpass->run();
        delete solver_meanpass;
	}
...
void potts_meanpass_2d_gpu_back(torch::Tensor g, torch::Tensor u, torch::Tensor rx, torch::Tensor ry, torch::Tensor g_data, torch::Tensor g_rx, torch::Tensor g_ry) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto grad_meanpass = new POTTS_MEANPASS_GRADIENT<CUDA_DEVICE>(dev, 2, data_sizes, n_c, inputs, outputs);
		grad_meanpass->run();
        delete grad_meanpass;
	}
...
void potts_meanpass_3d_gpu(torch::Tensor data, torch::Tensor rx, torch::Tensor ry, torch::Tensor rz, torch::Tensor out) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto solver_auglag = new POTTS_AUGLAG_SOLVER<CUDA_DEVICE>(dev, false, 3, data_sizes, n_c, buffers+1, buffers[0]);
		solver_auglag->run();
		exp(dev,u_init_buf,u_init_buf,n_s*n_c);
        delete solver_auglag;
		
		auto solver_meanpass = new POTTS_MEANPASS_SOLVER<CUDA_DEVICE>(dev, 3, data_sizes, n_c, buffers, out_buf+b*n_s*n_c);
		solver_meanpass->run();
        delete solver_meanpass;
	}
...
void potts_meanpass_3d_gpu_back(torch::Tensor g, torch::Tensor u, torch::Tensor rx, torch::Tensor ry, torch::Tensor rz, torch::Tensor g_data, torch::Tensor g_rx, torch::Tensor g_ry, torch::Tensor g_rz) {
	...
	for(int b = 0; b < n_b; b++){
		...
		auto grad_meanpass = new POTTS_MEANPASS_GRADIENT<CUDA_DEVICE>(dev, 3, data_sizes, n_c, inputs, outputs);
		grad_meanpass->run();
        delete grad_meanpass;
	}
}
...
void potts_gpu_bindings(py::module & m) {
  m.def("potts_auglag_1d_gpu_forward", &potts_auglag_1d_gpu, "deepflow potts_auglag_1d_gpu_forward");
  m.def("potts_auglag_2d_gpu_forward", &potts_auglag_2d_gpu, "deepflow potts_auglag_2d_gpu_forward");
  m.def("potts_auglag_3d_gpu_forward", &potts_auglag_3d_gpu, "deepflow potts_auglag_3d_gpu_forward");
  m.def("potts_meanpass_1d_gpu_forward", &potts_meanpass_1d_gpu, "deepflow potts_meanpass_1d_gpu_forward");
  m.def("potts_meanpass_2d_gpu_forward", &potts_meanpass_2d_gpu, "deepflow potts_meanpass_2d_gpu_forward");
  m.def("potts_meanpass_3d_gpu_forward", &potts_meanpass_3d_gpu, "deepflow potts_meanpass_3d_gpu_forward");
  m.def("potts_meanpass_1d_gpu_backward", &potts_meanpass_1d_gpu_back, "deepflow potts_meanpass_1d_gpu_backward");
  m.def("potts_meanpass_2d_gpu_backward", &potts_meanpass_2d_gpu_back, "deepflow potts_meanpass_2d_gpu_backward");
  m.def("potts_meanpass_3d_gpu_backward", &potts_meanpass_3d_gpu_back, "deepflow potts_meanpass_3d_gpu_backward");
}

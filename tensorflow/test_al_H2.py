...
class InnerProductOpTest(unittest.TestCase):
    def test_runAndPrintOutput(self):
        ...
        for i in range(1):
            ...
            input_rx = 1*np.random.rand(b,r,x,y)
            input_ry = 1*np.random.rand(b,r,x,y)
            #input_rx[:,:,:,:,2] *= 0
            #input_rx[:,:,:,:,3] *= 0
            #input_rx[:,:,:,:,4] *= 0
            #input_rx[:,:,:,:,5] *= 0
            ...
            for devicename in ['/CPU:0','/GPU:0']:
                ...
                with tf.device(devicename):
                    print("Doing auglag")
                    forward = meanpass_module.hmf_auglag2d(data,rx,ry,parentage,data_index)
                    print("Doing meanpass no init")
                    forward_mean = meanpass_module.hmf_meanpass2d(data,rx,ry,parentage,data_index)
                    print("Doing meanpass with init")
                    forward_mean_init = meanpass_module.hmf_meanpass2d_with_init(data,rx,ry,forward,parentage,data_index)

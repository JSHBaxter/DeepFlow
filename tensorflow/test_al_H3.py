...
class InnerProductOpTest(unittest.TestCase):
    def test_runAndPrintOutput(self):
        ...
        for i in range(1):
            input_d = 0.01*np.random.rand(b,c,x,y,z)
            input_rx = 1*np.random.rand(b,r,x,y,z)
            input_ry = 1*np.random.rand(b,r,x,y,z)
            input_rz = 1*np.random.rand(b,r,x,y,z)
            #input_rx[:,:,:,:,2] *= 0
            #input_rx[:,:,:,:,3] *= 0
            #input_rx[:,:,:,:,4] *= 0
            #input_rx[:,:,:,:,5] *= 0
            ...
            for devicename in ['/CPU:0','/GPU:0']:
                ...
                if devicename == '/CPU:0':
                    data = tf.convert_to_tensor(np.transpose(input_d,[0,2,3,4,1]),dtype=tf.float32)
                    rx = tf.convert_to_tensor(np.transpose(input_rx,[0,2,3,4,1]),dtype=tf.float32)
                    ry = tf.convert_to_tensor(np.transpose(input_ry,[0,2,3,4,1]),dtype=tf.float32)
                    rz = tf.convert_to_tensor(np.transpose(input_rz,[0,2,3,4,1]),dtype=tf.float32)
                else:
                    ...
                    rz = tf.convert_to_tensor(input_rz,dtype=tf.float32)
                    
                    
                with tf.device(devicename):
                    print("Doing auglag")
                    forward = meanpass_module.hmf_auglag3d(data,rx,ry,rz,parentage,data_index)
                    print("Doing meanpass no init")
                    forward_mean = meanpass_module.hmf_meanpass3d(data,rx,ry,rz,parentage,data_index)
                    print("Doing meanpass with init")
                    forward_mean_init = meanpass_module.hmf_meanpass3d_with_init(data,rx,ry,rz,forward,parentage,data_index)
                    ...
                    if devicename == '/CPU:0':
                        ...
                        forward_mean_init = np.transpose(forward_mean_init,[0,4,1,2,3])
                ...
                forward_mean = np.exp(forward_mean-forward_mean_max) / np.sum(np.exp(forward_mean-forward_mean_max),axis=1,keepdims=True)
                for i in range(c):
                    print( (np.round(forward_mean[0,i,:,:,:]*100)).astype(int) )
                print( '\n' )
                print( '\n' )
                
                forward_mean_max = np.max(forward_mean_init, axis=1, keepdims=True)
                forward_mean_init = np.exp(forward_mean_init-forward_mean_max) / np.sum(np.exp(forward_mean_init-forward_mean_max),axis=1,keepdims=True)
                for i in range(c):

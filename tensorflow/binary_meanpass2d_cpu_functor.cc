...
struct BinaryMeanpass2dFunctor<CPUDevice> {
  void operator()(
    ...
    for(int b = 0; b < n_batches; b++)
        threads[b] = new std::thread(BINARY_MEANPASS_CPU_SOLVER_2D(false, b, n_c, data_sizes,
																  data_cost+ b*n_s*n_c,
																  rx_cost+ b*n_s*n_c,
																  ry_cost+ b*n_s*n_c,
...
struct BinaryMeanpass2dGradFunctor<CPUDevice> {
  void operator()(
    ...
    std::thread** threads = new std::thread* [n_batches];
    for(int b = 0; b < n_batches; b++)
        threads[b] = new std::thread(BINARY_MEANPASS_CPU_GRADIENT_2D(false, b, n_c, data_sizes,
																	u+ b*n_s*n_c,
																	g+ b*n_s*n_c,
																	rx_cost+ b*n_s*n_c,
																	ry_cost+ b*n_s*n_c,

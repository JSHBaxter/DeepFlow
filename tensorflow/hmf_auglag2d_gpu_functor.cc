...
struct HmfAuglag2dFunctor<GPUDevice> {
    void operator()(
        ...
        for(int b = 0; b < n_batches; b++)
            HMF_AUGLAG_GPU_SOLVER_2D(d.stream(), bottom_up_list, b, n_c, n_r, data_sizes, 
                                     data_cost + b*n_s*n_c,
                                     rx_cost + b*n_s*n_r,
                                     ry_cost + b*n_s*n_r,
                                     u + b*n_s*n_c,
    ...
    int num_buffers_full(){ return HMF_AUGLAG_GPU_SOLVER_2D::num_buffers_full(); }
    int num_buffers_images(){ return HMF_AUGLAG_GPU_SOLVER_2D::num_buffers_images(); }
    int num_buffers_branch(){ return HMF_AUGLAG_GPU_SOLVER_2D::num_buffers_branch(); }
    int num_buffers_data(){ return HMF_AUGLAG_GPU_SOLVER_2D::num_buffers_data(); }
};

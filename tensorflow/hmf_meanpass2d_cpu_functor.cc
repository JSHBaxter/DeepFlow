...
struct HmfMeanpass2dFunctor<CPUDevice> {
  void operator()(
    ...
    for(int b = 0; b < n_batches; b++)
        threads[b] = new std::thread(HMF_MEANPASS_CPU_SOLVER_2D(false, bottom_up_list, b, n_c, n_r, data_sizes, 
                                                                data_cost + b*n_s*n_c,
																rx_cost + b*n_s*n_r,
                                                                ry_cost + b*n_s*n_r,
...
struct HmfMeanpass2dGradFunctor<CPUDevice> {
  void operator()(
    ...
    std::thread** threads = new std::thread* [n_batches];
    for(int b = 0; b < n_batches; b++)
        threads[b] = new std::thread(HMF_MEANPASS_CPU_GRADIENT_2D(false, bottom_up_list, n_c, n_r, data_sizes, 
                                                                  u + b*n_s*n_c,
                                                                  g + b*n_s*n_c,
                                                                  g_data + b*n_s*n_c,
                                                                  rx_cost + b*n_s*n_r,

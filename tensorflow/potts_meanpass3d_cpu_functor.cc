...
struct PottsMeanpass3dFunctor<CPUDevice> {
  void operator()(
    ...
    for(int b = 0; b < n_batches; b++)
        threads[b] = new std::thread(POTTS_MEANPASS_CPU_SOLVER_3D(false, b, n_c, data_sizes,
																  data_cost + b*n_s*n_c,
																  rx_cost + b*n_s*n_c,
																  ry_cost + b*n_s*n_c,
																  rz_cost + b*n_s*n_c,
...
struct PottsMeanpass3dGradFunctor<CPUDevice> {
  void operator()(
    ...
    std::thread** threads = new std::thread* [n_batches];
    for(int b = 0; b < n_batches; b++)
        threads[b] = new std::thread(POTTS_MEANPASS_CPU_GRADIENT_3D(false, b, n_c, data_sizes,
																	u + b*n_s*n_c,
																	g + b*n_s*n_c,
																	rx_cost + b*n_s*n_c,
																	ry_cost + b*n_s*n_c,
